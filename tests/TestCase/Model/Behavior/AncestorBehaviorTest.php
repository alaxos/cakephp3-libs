 ...
 */
	...
	{
		$query = $this->Nodes->getConnection()->query('SET FOREIGN_KEY_CHECKS=0;');
		$query = $this->Nodes->getConnection()->query('TRUNCATE ' . $this->Nodes->getTable() . ';');
		$query = $this->Nodes->getConnection()->query('TRUNCATE ' . $this->Nodes->getTable() . '_ancestors;');
		$query = $this->Nodes->getConnection()->query('SET FOREIGN_KEY_CHECKS=1;');
	}
 ...
 */
	...
	public function testGetMultilevelChildren(){
		...
		$multilevel_children_count = $multilevel_children->countBy(function($node) {
            return $node->parent_id = 1 ? 'direct' : 'other';
		})->toArray();

		$this->assertEquals(1, $multilevel_children_count['direct']);
 ...
 */
	...
	public function testMoveUnderItself()
	{
		...
		$this->Nodes->patchEntity($entity_3, ['parent_id' => 3]);
	 ...
	 */
	public function testMoveUnderItselfWithoutPatchEntity()
	{
	    $entity_3 = $this->Nodes->findById(3)->first();
	...
	{
		$entity_3 = $this->Nodes->findById(3)->first();
		$this->Nodes->patchEntity($entity_3, ['parent_id' => 7]);
	 ...
	 * @expectedException Exception
	 */
	public function testMoveUnderChildWithoutPatchEntity()
	{
	    $entity_3 = $this->Nodes->findById(3)->first();
