...
class AncestorBehavior extends Behavior
{
	...
	public function __construct(Table $table, array $config = []) {
		...
		$this->_defaultConfig['ancestor_table_name'] = $table->getSchema()->name() . '_ancestors';
	...
	public function addValidationRules(){
		...
		$add_validation_rules = $this->getConfig('add_validation_rules');
...
// 			                                                                     debug($context);
	...
	public function beforeSave(Event $event, Entity $entity) {
		...
		if(!$entity->isNew())
		{
			...
			$primaryKey = $table->getSchema()->primaryKey();
	...
	public function findChildren(Query $query, array $options){
		...
		if($options['direct'])
		{
			...
			if ($query->clause('order') === null) {
				$sort = !empty($options['sort']) ? $options['sort'] : [$this->getConfig('model_sort_fieldname') => 'asc'];
				$query->order($sort);
			}

			$query->where([$this->getConfig('model_parent_id_fieldname') => $for]);
		...
		else
		{
			...
			*/
			...
			$selected_fields = $this->_table->getSchema()->columns();
			...
			$query->select($selected_fields)
					->join([
							...
							'alias' => 't2',
							'type'  => 'INNER',
							'conditions' => $this->_table->getAlias() . '.id = t2.node_id'
							])
...
// 			return new \Cake\Collection\Collection($nodes);
	...
	public function findPath(Query $query, array $options)
	{
		...
		$model_alias = $this->_table->getAlias();
